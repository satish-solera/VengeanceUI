...
import { AnimatedTabBarDemo } from '@/components/docs/animated-tab-bar'
import { ComponentPreview } from '@/components/docs/component-preview'

# Animated Tab Bar

A highly animated tab bar using GSAP for SVG morphing and fluid interaction.

<ComponentPreview
  component={<AnimatedTabBarDemo />}
  code={`"use client";
...
<Dependencies step={1} title="Install dependencies">
    ...
    <p className="text-sm text-muted-foreground mt-4">
        <strong>Note:</strong> This component uses <code>GSAP MorphSVGPlugin</code> for the icon morphing animations. This is a paid plugin. Without it, the morphing will not occur, but other animations will work.
    </p>
</Dependencies>

<Dependencies step={2} title="Add CSS Styles">
    <p className="text-[13px] text-muted-foreground mt-2 mb-8">
        Create a file <code className="text-foreground font-mono text-xs bg-neutral-100 dark:bg-[#161616] border border-neutral-200 dark:border-neutral-800 px-2 py-1 rounded-md mx-1">components/ui/animated-tab-bar.css</code> and paste the following code:
    </p>
    <CodeBlock
        language="css"
        title="components/ui/animated-tab-bar.css"
        expandable={true}
        code={`.tab-bar {
    margin-top: auto;
    display: flex;
...
<Dependencies step={3} title="Copy the component code">
    <CodeBlock
        language="tsx"
        title="components/ui/animated-tab-bar.tsx"
        expandable={true}
        code={`"use client";

import React, { useEffect, useRef, useState } from "react";
import gsap from "gsap";
import "./animated-tab-bar.css";
